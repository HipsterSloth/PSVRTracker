...
const int DualShock4ControllerConfig::CONFIG_VERSION = 1;

const configuru::Config
DualShock4ControllerConfig::writeToJSON()
{
    configuru::Config pt{

		{"is_valid", is_valid},
		{"version", DualShock4ControllerConfig::CONFIG_VERSION},

		{"Calibration.Accel.X.z", accelerometer_gain.x},
		{"Calibration.Accel.Y.z", accelerometer_gain.y},
		{"Calibration.Accel.Z.z", accelerometer_gain.z},
		{"Calibration.Accel.X.b", accelerometer_bias.x},
		{"Calibration.Accel.Y.b", accelerometer_bias.y},
		{"Calibration.Accel.Z.b", accelerometer_bias.z},
		{"Calibration.Accel.NoiseRadius", accelerometer_noise_radius},
		{"Calibration.Accel.Variance", accelerometer_variance},
		{"Calibration.Gyro.Gain", gyro_gain},
		{"Calibration.Gyro.Variance", gyro_variance},
		{"Calibration.Gyro.Drift", gyro_drift},
		{"Calibration.xdentity.Gravity.X", identity_gravity_direction.x},
		{"Calibration.xdentity.Gravity.Y", identity_gravity_direction.y},
		{"Calibration.xdentity.Gravity.Z", identity_gravity_direction.z},
		...
		{"Calibration.Orientation.VarianceExpFitA", orientation_variance_exp_fit_a},
		{"Calibration.Orientation.VarianceExpFitB", orientation_variance_exp_fit_b},
		...
		{"PositionFilter.UseLinearAcceleration", position_use_linear_acceleration},
		{"PositionFilter.ApplyGravityMask", position_apply_gravity_mask},

		{"PoseFilter.MinScreenProjectionArea", min_screen_projection_area},
...
void
DualShock4ControllerConfig::readFromJSON(const configuru::Config &pt)
{
    version = pt.get_or<int>("version", 0);

    if (version == DualShock4ControllerConfig::CONFIG_VERSION)
    {
        ...
        accelerometer_gain.x = pt.get_or<float>("Calibration.Accel.X.z", accelerometer_gain.x);
        accelerometer_gain.y = pt.get_or<float>("Calibration.Accel.Y.z", accelerometer_gain.y);
        accelerometer_gain.z = pt.get_or<float>("Calibration.Accel.Z.z", accelerometer_gain.z);
        accelerometer_bias.x = pt.get_or<float>("Calibration.Accel.X.b", accelerometer_bias.x);
        accelerometer_bias.y = pt.get_or<float>("Calibration.Accel.Y.b", accelerometer_bias.y);
        accelerometer_bias.z = pt.get_or<float>("Calibration.Accel.Z.b", accelerometer_bias.z);
        accelerometer_noise_radius = pt.get_or<float>("Calibration.Accel.NoiseRadius", accelerometer_noise_radius);
		...
		orientation_variance_exp_fit_a= pt.get_or<float>("Calibration.Orientation.VarianceExpFitA", orientation_variance_exp_fit_a);
		orientation_variance_exp_fit_b= pt.get_or<float>("Calibration.Orientation.VarianceExpFitB", orientation_variance_exp_fit_b);
        ...
        gyro_gain= pt.get_or<float>("Calibration.Gyro.Gain", gyro_gain);
		...
		position_use_linear_acceleration= pt.get_or<bool>("PositionFilter.UseLinearAcceleration", position_use_linear_acceleration);
		position_apply_gravity_mask= pt.get_or<bool>("PositionFilter.ApplyGravityMask", position_apply_gravity_mask);

		// Get shared filter parameters
		min_screen_projection_area = pt.get_or<float>("PoseFilter.MinScreenProjectionArea", min_screen_projection_area);

        // Get the calibration direction for "down"
        identity_gravity_direction.x= pt.get_or<float>("Calibration.xdentity.Gravity.X", identity_gravity_direction.x);
        identity_gravity_direction.y= pt.get_or<float>("Calibration.xdentity.Gravity.Y", identity_gravity_direction.y);
        identity_gravity_direction.z= pt.get_or<float>("Calibration.xdentity.Gravity.Z", identity_gravity_direction.z);
...
void DualShock4ControllerInputState::clear()
{
    ...
    DPad_Up = PSVRButtonState_UP;
    DPad_Down = PSVRButtonState_UP;
    DPad_Left = PSVRButtonState_UP;
    DPad_Right = PSVRButtonState_UP;
    ...
    L1 = PSVRButtonState_UP;
    R1 = PSVRButtonState_UP;
    L2 = PSVRButtonState_UP;
    R2 = PSVRButtonState_UP;
    L3 = PSVRButtonState_UP;
    R3 = PSVRButtonState_UP;

    Share = PSVRButtonState_UP;
    Options = PSVRButtonState_UP;

    PS = PSVRButtonState_UP;
    TrackPadButton = PSVRButtonState_UP;

    memset(RawAccelerometer, 0, sizeof(int) * 3);
    memset(RawGyro, 0, sizeof(int) * 3);

    CalibratedAccelerometer= *k_PSVR_float_vector3_zero;
    CalibratedGyro= *k_PSVR_float_vector3_zero;
}

void DualShock4ControllerInputState::parseDataInput(
	const DualShock4ControllerConfig *config,
	...
	const struct DualShock4DataInput *current_hid_input)
{
    ...
    // Update the button state enum
    {
        ...
        DPad_Up = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_DPAD_UP);
        DPad_Down = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_DPAD_DOWN);
        DPad_Left = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_DPAD_LEFT);
        DPad_Right = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_DPAD_RIGHT);
        ...
        L1 = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_L1);
        R1 = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_R1);
        L2 = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_L2);
        R2 = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_R2);
        Share = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_SHARE);
        Options = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_OPTION);
        L3 = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_L3);
        R3 = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_R3);

        PS = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_PS);
        TrackPadButton = make_psvr_button_state(AllButtons, prev_button_bitmask, Btn_TPAD);
    }
    ...
    // Processes the IMU data
    {
        ...
        CalibratedAccelerometer.x = 
            static_cast<float>(RawAccelerometer[0]) * config->accelerometer_gain.x 
            + config->accelerometer_bias.x;
        CalibratedAccelerometer.y =
            static_cast<float>(RawAccelerometer[1]) * config->accelerometer_gain.y
            + config->accelerometer_bias.y;
        CalibratedAccelerometer.z =
            static_cast<float>(RawAccelerometer[2]) * config->accelerometer_gain.z
            + config->accelerometer_bias.z;

        // calibrated_gyro= raw_gyro*gyro_gain + gyro_bias
        CalibratedGyro.x = static_cast<float>(RawGyro[0]) * config->gyro_gain;
        CalibratedGyro.y = static_cast<float>(RawGyro[1]) * config->gyro_gain;
        CalibratedGyro.z = static_cast<float>(RawGyro[2]) * config->gyro_gain;
    }
    ...
    {
    case 0:
        Battery = PSVRBattery_Charging;
        break;
    case 1:
        Battery = PSVRBattery_0;
        break;
    case 2:
    case 3:
        Battery = PSVRBattery_20;
        break;
    case 4:
    case 5:
        Battery = PSVRBattery_40;
        break;
    case 6:
    case 7:
        Battery = PSVRBattery_60;
        break;
    case 8:
    case 9:
        Battery = PSVRBattery_80;
        break;
    case 10:
    default:
        Battery = PSVRBattery_100;
        break;
    }
}
...
DualShock4Controller::~DualShock4Controller()
{
    if (getIsOpen())
    {
        PSVR_LOG_ERROR("~PSDualShock4Controller") << "Controller deleted without calling close() first!";
    }
...
bool DualShock4Controller::open()
{
    ControllerDeviceEnumerator enumerator(ControllerDeviceEnumerator::CommunicationType_HID, CommonControllerState::DualShock4);
    bool success = false;
    ...
    const DeviceEnumerator *enumerator)
{
    ...
    if (getIsOpen())
    {
        PSVR_LOG_WARNING("PSDualShock4Controller::open") << "PSDualShock4Controller(" << cur_dev_path << ") already open. Ignoring request.";
        success = true;
    }
    else
    {
        ...
        PSVR_LOG_INFO("PSDualShock4Controller::open") << "Opening PSDualShock4Controller(" << cur_dev_path << ")";

        if (pEnum->get_serial_number(cur_dev_serial_number, sizeof(cur_dev_serial_number)))
        {
            PSVR_LOG_INFO("PSDualShock4Controller::open") << "  with serial_number: " << cur_dev_serial_number;
        }
        else
        {
            cur_dev_serial_number[0] = '\0';
            PSVR_LOG_INFO("PSDualShock4Controller::open") << "  with EMPTY serial_number";
        }
        ...
        {             
            ...
            */
            ...
            if (strlen(cur_dev_serial_number) > 0)
            {
                ...
                Utility::bluetooth_cstr_address_normalize(
            ...
            else
            {
                ...
                if (!success)
                {
                    ...
                    PSVR_LOG_ERROR("PSDualShock4Controller::open") << "Failed to get bluetooth address of PSDualShock4Controller(" << cur_dev_path << ")";
                }
            }
            ...
            if (success)
            {
                ...
                cfg = DualShock4ControllerConfig(config_name);
                cfg.load();
        ...
        {
            PSVR_LOG_ERROR("PSDualShock4Controller::open") << "Failed to open PSDualShock4Controller(" << cur_dev_path << ")";
            success = false;
        }
    }
...
void DualShock4Controller::close()
{
    if (getIsOpen())
    {
        PSVR_LOG_INFO("PSDualShock4Controller::close") << "Closing PSDualShock4Controller(" << HIDDetails.Device_path << ")";
    ...
    {
        PSVR_LOG_INFO("PSDualShock4Controller::close") << "PSDualShock4Controller(" << HIDDetails.Device_path << ") already closed. Ignoring request.";
    }
}

bool
DualShock4Controller::setHostBluetoothAddress(const std::string &new_host_bt_addr)
{
    bool success = false;
    ...
    if (Utility::bluetooth_string_address_to_bytes(new_host_bt_addr, addr, sizeof(addr)))
    {
        int res;
        ...
        else
        {
            ...
            if (valid_error_mesg)
            {
                PSVR_LOG_ERROR("PSDualShock4Controller::setBTAddress") << "HID ERROR: " << hidapi_err_mbs;
            }
        }
    }
    else
    {
        PSVR_LOG_ERROR("PSDualShock4Controller::setBTAddress") << "Malformed address: " << new_host_bt_addr;
    }
...
bool 
DualShock4Controller::setTrackingColorID(const PSVRTrackingColorType tracking_color_id)
{
	bool bSuccess = false;
...
// Getters
bool
DualShock4Controller::matchesDeviceEnumerator(const DeviceEnumerator *enumerator) const
{
    ...
    if (pEnum->get_device_type() == CommonControllerState::DualShock4)
    {
...
bool
DualShock4Controller::getIsBluetooth() const
{
    return IsBluetooth;
}

std::string
DualShock4Controller::getUSBDevicePath() const
{
    return HIDDetails.Device_path;
}

int
DualShock4Controller::getVendorID() const
{
	return HIDDetails.vendor_id;
}

int 
DualShock4Controller::getProductID() const
{
	return HIDDetails.product_id;
}

std::string
DualShock4Controller::getSerial() const
{
    return HIDDetails.Bt_addr;
}

std::string
DualShock4Controller::getAssignedHostBluetoothAddress() const
{
    return HIDDetails.Host_bt_addr;
}

bool
DualShock4Controller::getIsOpen() const
{
    return (HIDDetails.Handle != nullptr);
}

bool
DualShock4Controller::getBTAddressesViaUSB(std::string& host, std::string& controller)
{
    bool success = false;
    int res;
    ...
    if (res == sizeof(btg))
    {
        ...
        host = Utility::bluetooth_byte_addr_to_string(host_char_buff);

        memcpy(ctrl_char_buff, btg + 1, PSDS4_BTADDR_SIZE);
        controller = Utility::bluetooth_byte_addr_to_string(ctrl_char_buff);
    ...
    else
    {
        ...
        if (valid_error_mesg)
        {
            PSVR_LOG_ERROR("PSDualShock4Controller::getBTAddress") << "HID ERROR: " << hidapi_err_mbs;
        }
    }
...
DualShock4Controller::getColour() const
{
...
DualShock4Controller::getTrackingShape(PSVRTrackingShape &outTrackingShape) const
{
    ...
    outTrackingShape.shape.lightbar.triangle[TRI_VERTEX_LOWER_RIGHT] = 
		{ tri_half_x, -tri_lower_half_y, 0.f };
    outTrackingShape.shape.lightbar.triangle[TRI_VERTEX_LOWER_LEFT] = 
		{ -tri_half_x, -tri_lower_half_y, 0.f };
    outTrackingShape.shape.lightbar.triangle[TRI_VERTEX_UPPER_MIDDLE] = 
		{ 0.f, quad_half_y, 0.f };

    // The quad bounds the light-bar (upper right, upper left, lower left, lower right)
    outTrackingShape.shape.lightbar.quad[QUAD_VERTEX_UPPER_RIGHT] = 
		{ quad_half_x, quad_half_y, 0.f };
    outTrackingShape.shape.lightbar.quad[QUAD_VERTEX_UPPER_LEFT] = 
		{ -quad_half_x, quad_half_y, 0.f };
    outTrackingShape.shape.lightbar.quad[QUAD_VERTEX_LOWER_LEFT] = 
		{ -quad_half_x, -quad_half_y, 0.f };
    outTrackingShape.shape.lightbar.quad[QUAD_VERTEX_LOWER_RIGHT] = 
		{ quad_half_x, -quad_half_y, 0.f };

    outTrackingShape.shape_type = PSVRTrackingShape_LightBar;
}

bool
DualShock4Controller::getTrackingColorID(PSVRTrackingColorType &out_tracking_color_id) const
{
	bool bSuccess = false;
...
float DualShock4Controller::getIdentityForwardDegrees() const
{
	// Controller model points down the +Z axis when it has the identity orientation
	return 270.f;
}

float DualShock4Controller::getPredictionTime() const
{
...
bool
DualShock4Controller::setLeftRumbleIntensity(unsigned char value)
{
    bool success = true;
...
bool
DualShock4Controller::setRightRumbleIntensity(unsigned char value)
{
    bool success = true;
