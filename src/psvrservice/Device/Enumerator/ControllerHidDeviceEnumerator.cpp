...
const int MAX_HID_CONTROLLER_TYPE_COUNT= 3;
...
bool ControllerHidDeviceEnumerator::get_serial_number(char *out_mb_serial, const size_t mb_buffer_size) const
{
	...
	{
		success = Utility::convert_wcs_to_mbs(m_currentHIDDevice->serial_number, out_mb_serial, mb_buffer_size);
	}
...
bool ControllerHidDeviceEnumerator::is_valid() const
{
	...
	**/
	if (bIsValid && m_deviceType == CommonSensorState::PSMove && strstr(m_currentHIDDevice->path, "&col01#") == nullptr)
	{
		bIsValid = false;
	}
#endif
		...
		if (!foundValid && m_currentHIDDevice == nullptr)
		{
			...
			if (m_enumeratorIndex < MAX_HID_CONTROLLER_TYPE_COUNT)
			{
				...
				if (dev_info.bHIDApiSupported &&
					(m_deviceType == m_deviceTypeFilter || m_deviceTypeFilter == CommonSensorState::INVALID_DEVICE_TYPE))
				{
