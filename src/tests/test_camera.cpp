...
int main(int, char**)
{
	...
	for (int camera_index = 0; camera_index < PSVRSERVICE_MAX_TRACKER_COUNT; ++camera_index)
	{
        ...
        if (camera->isOpened())
        {
			...
			if (camera->get(cv::CAP_PROP_FRAME_WIDTH) != frame_width_init) 
				camera->set(cv::CAP_PROP_FRAME_WIDTH, frame_width_init);

			if (camera->get(cv::CAP_PROP_FPS) != frame_rate_init)
				camera->set(cv::CAP_PROP_FPS, frame_rate_init);
    ...
    while (bKeepRunning)
    {
        ...
        {
            int cap_prop = cv::CAP_PROP_EXPOSURE;
            double val_diff = 0;
            ...
            if ((wk == 116) || (wk == 103))
            {
                cap_prop = cv::CAP_PROP_SHARPNESS;
			...
			if ((wk == 117) || (wk == 106))
			{
				cap_prop = cv::CAP_PROP_FRAME_WIDTH;
            ...
            std::for_each(
                ...
                [&camera_states, &cap_prop, &prop_str, &val_diff](camera_state &state) {
					...
					case cv::CAP_PROP_FPS:
						int frame_width = state.camera->get(cv::CAP_PROP_FRAME_WIDTH);
						if (frame_width == 640)
						{
