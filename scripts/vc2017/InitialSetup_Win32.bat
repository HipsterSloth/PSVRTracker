...
MSBuild.exe libusb_2017.sln /tv:15.0 /p:configuration=DEBUG /p:Platform="Win32" /t:Clean;Build
IF %ERRORLEVEL% NEQ 0 (
  echo "Error building libusb DEBUG|Win32!"
) 
echo "Building libusb RELEASE|Win32..."
MSBuild.exe libusb_2017.sln /tv:15.0 /p:configuration=RELEASE /p:Platform="Win32" /t:Clean;Build
IF %ERRORLEVEL% NEQ 0 (
...
mkdir build
pushd build
cmake .. -G "Visual Studio 15 2017" -DDIRECTX=OFF -DDIRECTX=OFF -DSDL_STATIC=ON -DSDL_SHARED=OFF -DFORCE_STATIC_VCRT=ON -DEXTRA_CFLAGS="-MT -Z7 -DSDL_MAIN_HANDLED -DWIN32 -DNDEBUG -D_CRT_SECURE_NO_WARNINGS -DHAVE_LIBC -D_USE_MATH_DEFINES
IF %ERRORLEVEL% NEQ 0 (
